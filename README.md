...
// Comprehensive City and Brand Search Data
const cityData = {
  cities: [
    "Mumbai", "Delhi", "Jaipur", "Bangalore", "Hyderabad", "Chennai", "Kolkata"
  ],
  categories: [
    {
      name: "Food",
  ...
  outlets: [
    {
      ...
      ratings: 4.5,
      menu: [
        { item: "Margherita Pizza", price: 199 },
        { item: "Pepperoni Pizza", price: 249 }
      ],
      location: {
    ...
    // More outlets can be added here
  ]
};

// City, Brand, and Category Selection Overlay
function showBrandSearchOverlay() {
  let overlay = document.getElementById('brandSearchOverlay');
  if (!overlay) {
    overlay = document.createElement('div');
    overlay.id = 'brandSearchOverlay';
    overlay.className = 'brand-search-overlay';
    document.body.appendChild(overlay);
  }
  
  overlay.innerHTML = `
    <div class="brand-search-content">
      <div class="brand-search-header">
        <h2>Find Places</h2>
        <div class="header-actions">
          <button class="select-town-btn">Select Town</button>
          <button class="close-brand-search" onclick="hideBrandSearchOverlay()">
            <i class="fas fa-times"></i>
          </button>
        </div>
      </div>
        
            <div class="brand-search-filters">
        <div class="filter-section">
          ...
          <div class="category-selection">
            <select id="categorySelect" class="form-select">
              ...
              ${cityData.categories.map(category => `<option value="${category.name}">${category.name}</option>`).join('')}
            </select>
          </div>
          ...
          <button class="search-brands-btn" onclick="searchOutlets()">
  ...
  const style = document.createElement('style');
  style.textContent = `
    /* Similar styling to previous overlay with additional refinements */
...
.select-town-btn: var(--primary-color); 
  color: black; 
}
    ...
    /* Add more detailed styles for outlet cards */
  `;
...
function setupSelectionListeners() {
  ...
  categorySelect.addEventListener('change', function() {
    ...
    if (selectedCategory) {
      const category = cityData.categories.find(cat => cat.name === selectedCategory);
...
function searchOutlets() {
  ...
  const filteredOutlets = cityData.outlets.filter(outlet => 
    (!city || outlet.city === city) &&
    (!category || outlet.category === category) &&
    (!subCategory || outlet.subCategory === subCategory) &&
    (!brand || outlet.name === brand)
  );
  ...
  filteredOutlets.forEach(outlet => {
    ...
    outletCard.innerHTML = `
      <div class="outlet-image">
        <img src="${outlet.images[0]}" alt="${outlet.name}">
      </div>
      <div class="outlet-details">
        <h3>${outlet.name}</h3>
        <p>${outlet.description}</p>
        <div class="outlet-meta">
          <span class="category-tag">${outlet.categoryTags.join(', ')}</span>
          <span class="timing"><i class="fas fa-clock"></i> ${outlet.timings}</span>
        </div>
        <div class="outlet-actions">
          <button class="deals-btn"><i class="fas fa-percent"></i> Deals</button>
          <button class="menu-btn"><i class="fas fa-menu"></i> Menu</button>
          <button class="order-btn">Order Now</button>
        </div>
        <div class="outlet-ratings">
          <span class="rating"><i class="fas fa-star"></i> ${outlet.ratings}</span>
          <span class="delivery-status">
            ${outlet.deliveryAvailable ? 'Delivery Available' : 'Pickup Only'}
          </span>
        </div>
      </div>
    `;
    
    searchResults.appendChild(outletCard);
  });
}

function hideBrandSearchOverlay() {
  const overlay = document.getElementById('brandSearchOverlay');
  if (overlay) {
    overlay.classList.remove('active');
    setTimeout(() => overlay.remove(), 300);
  }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (townButton) {
        townButton.addEventListener('click', function(e) {
            ...
            hideTownsOverlay(); // Close the towns overlay
            showBrandSearchOverlay(); // Open the brand search overlay
        });
    }
});
